...
namespace ets2_la_plugin::prism
{
    ...
    class route_task_t : public thread::task_t // Size: 0x0148
    {
    public:
        char pad_0038[ 16 ];                                        // 0x0038 (0x10)
        array_dyn_t< physical_route_item_t > physical_route_items;  // 0x0048 (0x20) node to node
        array_dyn_t< route_item_t > route_items;                    // 0x0068 (0x20)
        char pad_0088[ 24 ];                                        // 0x0088 (0x18)
        array_dyn_t< token_t > N00007005;                           // 0x00A0 (0x20)
        char pad_00C0[ 8 ];                                         // 0x00C0 (0x08)
        array_dyn_t< class private_route_task_target_t > N000071A4; // 0x00C8 (0x20)
        char pad_00E8[ 24 ];                                        // 0x00E8 (0x18)
        array_dyn_t< uint32_t > N00007011;                          // 0x0100 (0x20)
        char pad_0120[ 40 ];                                        // 0x0120 (0x28)
    };
    static_assert( sizeof( route_task_t ) == 0x148 );
